...
h_zumaia=z[-1,-1].astype(np.float64)


xT_max=df_comp.tmax.values.astype(np.float64)
yT_max=tempcorr_ERA5Land(df_comp.tmax_era5L.values.astype(np.float64),h_zumaia)
...
yT_min=tempcorr_ERA5Land(df_comp.tmin_era5L.values.astype(np.float64),h_zumaia)
...
ofile=open("Tdata_ZUMAIA_sesgocorr.dat",'w')
ofiles.append(ofile)
