...
set label at graph 0.1,0.87 "{/:Bold 28.3 ({/Symbol \260}C)}"
...
p 'Tdata_BILBO_AERO_sesgocorr.dat' u 1:2 w p lt 7 ps 1 lc rgb "red" notitle,\
  ymax(x) w l lw 3 t "T_{ERA5} = 0.09 + 0.81T_{OBS} ({/Symbol \260}C)",28.30 lw 2 lc rgb "black" notitle
...
set label at graph 0.14,0.77 "{/:Bold 16.4 ({/Symbol \260}C)}"
...
p 'Tdata_BILBO_AERO_sesgocorr.dat' u 3:4 w p lt 7 ps 1 lc rgb "red" notitle,\
  ymin(x) w l lw 3 t "T_{ERA5} = 2.22 + 0.83T_{OBS} ({/Symbol \260}C)",16.37 lw 2 lc rgb "black" notitle
