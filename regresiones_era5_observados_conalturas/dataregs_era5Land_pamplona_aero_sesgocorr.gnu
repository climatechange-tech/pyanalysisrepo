...
set label at graph 0.1,0.85 "{/:Bold 31 ({/Symbol \260}C)}"
...
p 'Tdata_FORONDA_sesgocorr.dat' u 1:2 w p lt 7 ps 1 lc rgb "red" notitle,\
  ymax(x) w l lw 3 t "T_{ERA5} = -0.03 + 0.89T_{OBS} ({/Symbol \260}C)",31.04 lw 2 lc rgb "black" notitle
...
set label at graph 0.1,0.85 "{/:Bold 16.82 ({/Symbol \260}C)}"
...
p 'Tdata_FORONDA_sesgocorr.dat' u 3:4 w p lt 7 ps 1 lc rgb "red" notitle,\
  ymin(x) w l lw 3 t "T_{ERA5} = 0.78 + 0.94T_{OBS} ({/Symbol \260}C)",16.82 lw 2 lc rgb "black" notitle
