...
h_gue=z[1,-1].astype(np.float64)

xT_max=df_comp.tmax.values.astype(np.float64)
yT_max=tempcorr_ERA5Land(df_comp.tmax_era5L.values.astype(np.float64),h_gue)
...
yT_min=tempcorr_ERA5Land(df_comp.tmin_era5L.values.astype(np.float64),h_gue)
...
ofile=open("Tdata_GUENES_sesgocorr.dat",'w')
ofiles.append(ofile)
