...
set label at graph 0.1,0.83 "{/:Bold 29.9 ({/Symbol \260}C)}"
...
p 'Tdata_ZUMARRAGA_sesgocorr.dat' u 1:2 w p lt 7 ps 1 lc rgb "red" notitle,\
  ymax(x) w l lw 3 t "T_{ERA5} = -2.25 + 0.92T_{OBS} ({/Symbol \260}C)",29.90 lw 2 lc rgb "black" notitle
...
set label at graph 0.1,0.79 "{/:Bold 14.2 ({/Symbol \260}C)}"
...
p 'Tdata_ZUMARRAGA_sesgocorr.dat' u 3:4 w p lt 7 ps 1 lc rgb "red" notitle,\
  ymin(x) w l lw 3 t "T_{ERA5} = -2.61 + 0.99T_{OBS} ({/Symbol \260}C)",14.24 lw 2 lc rgb "black" notitle
