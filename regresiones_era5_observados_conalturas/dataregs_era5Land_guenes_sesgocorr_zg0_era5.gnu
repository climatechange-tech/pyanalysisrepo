...
set out "regs_era5Land_guenes_sesgocorr_zg0_era5.png"
...
set label at graph 0.1,0.82 "{/:Bold 29 ({/Symbol \260}C)}"
...
fit ymax(x) 'Tdata_GUENES_sesgocorr_zg0_era5.dat' u 1:2 via A,B

p 'Tdata_GUENES_sesgocorr_zg0_era5.dat' u 1:2 w p lt 7 ps 1 lc rgb "red" notitle,\
  ymax(x) w l lw 3 t "T_{ERA5} = -3.88 + 0.94T_{OBS} ({/Symbol \260}C)",28.97 lw 2 lc rgb "black" notitle
...
set label at graph 0.64,0.4 "{/:Bold r_{Tmin} = 0.96}"
set label at graph 0.1,0.8 "{/:Bold 12.8 ({/Symbol \260}C)}"
...
fit ymin(x) 'Tdata_GUENES_sesgocorr_zg0_era5.dat' u 3:4 via C,D

p 'Tdata_GUENES_sesgocorr_zg0_era5.dat' u 3:4 w p lt 7 ps 1 lc rgb "red" notitle,\
  ymin(x) w l lw 3 t "T_{ERA5} = -3.91 + 0.98T_{OBS} ({/Symbol \260}C)",12.83 lw 2 lc rgb "black" notitle
