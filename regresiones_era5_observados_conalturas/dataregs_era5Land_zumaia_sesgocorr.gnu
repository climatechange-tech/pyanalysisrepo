...
set label at graph 0.1,0.81 "{/:Bold 28.3 ({/Symbol \260}C)}"
...
p 'Tdata_ZUMAIA_sesgocorr.dat' u 1:2 w p lt 7 ps 1 lc rgb "red" notitle,\
  ymax(x) w l lw 3 t "T_{ERA5} = -3.60 + 1.06T_{OBS} ({/Symbol \260}C)",28.34 lw 2 lc rgb "black" notitle
...
set label at graph 0.1,0.83 "{/:Bold 16.8 ({/Symbol \260}C)}"
...
p 'Tdata_ZUMAIA_sesgocorr.dat' u 3:4 w p lt 7 ps 1 lc rgb "red" notitle,\
  ymin(x) w l lw 3 t "T_{ERA5} = -1.53 + 0.96T_{OBS} ({/Symbol \260}C)",16.76 lw 2 lc rgb "black" notitle
