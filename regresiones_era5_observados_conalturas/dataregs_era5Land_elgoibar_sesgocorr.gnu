...
set label at graph 0.1,0.87 "{/:Bold 31.1 ({/Symbol \260}C)}"
...
p 'Tdata_ELGOIBAR_sesgocorr.dat' u 1:2 w p lt 7 ps 1 lc rgb "red" notitle,\
  ymax(x) w l lw 3 t "T_{ERA5} = -5.25 + 1.04T_{OBS} ({/Symbol \260}C)",31.05 lw 2 lc rgb "black" notitle
...
set label at graph 0.67,0.35 "{/:Bold r_{Tmin} = 0.95}"
set label at graph 0.1,0.72 "{/:Bold 13.1 ({/Symbol \260}C)}"
...
p 'Tdata_ELGOIBAR_sesgocorr.dat' u 3:4 w p lt 7 ps 1 lc rgb "red" notitle,\
  ymin(x) w l lw 3 t "T_{ERA5} = -3.94 + 1.00T_{OBS} ({/Symbol \260}C)",13.06 lw 2 lc rgb "black" notitle
