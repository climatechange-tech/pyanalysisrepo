...
set out "regs_era5Land_zumarraga_sesgocorr_zg0_era5.png"
...
fit ymax(x) 'Tdata_ZUMARRAGA_sesgocorr_zg0_era5.dat' u 1:2 via A,B

p 'Tdata_ZUMARRAGA_sesgocorr_zg0_era5.dat' u 1:2 w p lt 7 ps 1 lc rgb "red" notitle,\
  ymax(x) w l lw 3 t "T_{ERA5} = -2.80 + 0.92T_{OBS} ({/Symbol \260}C)",29.35 lw 2 lc rgb "black" notitle
...
set key at graph 0.39,0.6

unset label
set xlabel "{/=14 AEMET observed temperature ({/Symbol \260}C)}"
set ylabel "{/=14 ERA5 LAND temperature ({/Symbol \260}C)}"

set label at graph 0.57,0.22 "{/:Bold r_{Tmin} = 0.96}"
set label at graph 0.1,0.76 "{/:Bold 13.4 ({/Symbol \260}C)}"
...
fit ymin(x) 'Tdata_ZUMARRAGA_sesgocorr_zg0_era5.dat' u 3:4 via C,D

p 'Tdata_ZUMARRAGA_sesgocorr_zg0_era5.dat' u 3:4 w p lt 7 ps 1 lc rgb "red" notitle,\
  ymin(x) w l lw 3 t "T_{ERA5} = -3.44 + 0.99T_{OBS} ({/Symbol \260}C)",13.41 lw 2 lc rgb "black" notitle
