...
set label at graph 0.1,0.8 "{/:Bold 27.5({/Symbol \260}C)}"
...
p 'Tdata_IGELDO_sesgocorr.dat' u 1:2 w p lt 7 ps 1 lc rgb "red" notitle,\
  ymax(x) w l lw 3 t "T_{ERA5} = -2.83 + 1.01T_{OBS} ({/Symbol \260}C)",27.51 lw 2 lc rgb "black" notitle
...
set label at graph 0.13,0.84 "{/:Bold 15.4 ({/Symbol \260}C)}"
...
p 'Tdata_IGELDO_sesgocorr.dat' u 3:4 w p lt 7 ps 1 lc rgb "red" notitle,\
  ymin(x) w l lw 3 t "T_{ERA5} = -3.32 + 0.98T_{OBS} ({/Symbol \260}C)",15.35 lw 2 lc rgb "black" notitle
